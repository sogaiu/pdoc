...
(import ../parse/questions :prefix "")
(import ../parse/tests :prefix "")
(import ../random :prefix "")

(defn qu/print-nicely
  [expr-str]
  (let [buf (hl/colorize (indent/format expr-str))]
    (each line (string/split "\n" buf)
      (print line))))

(defn qu/print-separator
  []
...
(defn qu/special-fill-in-quiz
  ...
  (let [[ques-zloc ans-zloc] (rnd/choose test-zloc-pairs)
        [blank-ques-zloc blanked-item] (qs/rewrite-test-zloc ques-zloc)]
    ...
    (let [ques (tests/indent-node-gen ques-zloc)
      ...
      # show the question
      (qu/print-nicely blank-ques)
      (print "# =>")
      (qu/print-nicely trimmed-ans)
      (print)
      # ask for an answer
      (def buf
      ...
      (print)
      #
      (qu/handle-fill-in-response ques blank-ques blanked-item
                               trimmed-ans resp))))

(defn qu/special-quiz
  [content]
  (def quiz-fn
    (rnd/choose [qu/special-plain-quiz
                 qu/special-fill-in-quiz]))
  (quiz-fn content))
