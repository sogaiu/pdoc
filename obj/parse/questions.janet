...
(defn qs/find-peg-specials
  ...
  (while (not (j/end? curr-zloc))
    (match (j/node curr-zloc)
      [:symbol _ content]
      (when (qs/is-peg-special? content)
...
(comment
  ...
  (map |(j/node $)
       (qs/find-peg-specials (-> (l/par src)
                              j/zip-down)))
  # =>
...
(comment
  ...
  (def ps-zloc
    (first (qs/find-peg-specials (-> (l/par src)
                                  j/zip-down))))

  (def [new-peg-special blanked-item]
    (qs/blank-peg-special ps-zloc))
...
(comment
  ...
  (j/node (qs/find-grammar-argument (->> (l/par src)
                                      j/zip-down)))
  # =>
  '(:quasiquote
...
(comment
  ...
  (j/node (qs/find-peg-match-call (->> (l/par src)
                                    j/zip-down)))
  # ->
  '(:tuple
...
(defn qs/rewrite-test-zloc
  [test-zloc]
  # XXX: why is this printing a function...
  (qs/dprintf "%M" j/path)
  (when-let [pm-call-zloc
             (qs/find-peg-match-call test-zloc)
             grammar-zloc
             (qs/find-grammar-argument pm-call-zloc)]
    (qs/dprintf "test:")
    (qs/dprintf (l/gen (j/node test-zloc)))
    (qs/dprintf "grammar:")
    (qs/dprintf (l/gen (j/node grammar-zloc)))
    ...
    (cond
      ...
      (do
        (qs/dprintf "grammar was a %s" grammar-node-type)
      ...
      (let [specials (qs/find-peg-specials grammar-zloc)]
        # XXX
        (qs/dprintf "grammar was a %s" grammar-node-type)
        # XXX
        (qs/dprintf "Number of specials found: %d" (length specials))
        (when (empty? specials)
          # XXX
          (eprint "Failed to find a special")
          (break [nil nil]))
        (each sp specials
          (qs/dprintf (l/gen (j/node sp))))
        (set chosen-special-zloc
             (rnd/choose specials))
        (qs/dprintf "chosen: %s" (l/gen (j/node chosen-special-zloc))))
      #
      (do
    ...
    (while curr-zloc
      ...
      (++ steps))
    # XXX
    (qs/dprintf "steps: %d" steps)
    # XXX: check not nil?
    (var [curr-zloc blanked-item]
      (->> chosen-special-zloc
           qs/blank-peg-special))
...
(defn qs/rewrite-test
  [test-zloc]
  (when-let [[rewritten-zloc blanked-item]
             (qs/rewrite-test-zloc test-zloc)]
    [(->> rewritten-zloc
         j/root
         l/gen)
     blanked-item]))
